...
def main(params):
    ...
    model_name = "mdn_i{:d}o{:d}_sp{:d}{:d}_n{:d}_{:s}".format(h_prev, h_post, sp_x, sp_y, n_component_gmm, txt_type)

    # LOAD MODEL PARAMETERS -------------------------------------------------------------------------------------------#
    directory2load = "{:s}/mdn".format(directory_model)
    ...
    if trackname_track == "us101" or trackname_track == "i80":
        vehicle_data_filename = "{:s}/dv_ngsim_{:s}_{:s}.npz".format(directory_vehicle, trackname_track.lower(), trackname_id)
        is_track_simple = 0
    ...
    for t_cur in range(t_init, t_init + t_horizon + 1):
        ...
        y_tv_pred = []
        valpred_y_list = nn_pred.sample(x_input, f_input, i_input, num_sample=n_sample)
        ...
        _, _y_mu, _, _y_log_pi = nn_pred.get_mdn_gmmdiag(x_input, f_input, i_input)
        _y_mu, _y_log_pi = _y_mu.numpy(), _y_log_pi.numpy()
        for nidx_i in range(0, n_component_gmm):
