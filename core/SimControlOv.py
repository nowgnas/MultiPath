...
class SimControlOv(object):
    """
    OTHER-VEHICLE FOR SIMULATOR
        ...
        dot(v) = kappa_2 * a
    """
    def __init__(self, dt_in, rx_in, ry_in, kappa, v_ref, v_range):
    ...
    def set_initstate(self, track, data_ov, t, segidx, laneidx, margin_rx, margin_ry, vel_rand=False):
        """
        Sets intial states.
        ...
        succeed_init, x_init, y_init, theta_init, pnts_init = \
            set_initstate(data_ov, t, self.rx, self.ry, segidx, laneidx, margin_rx, margin_ry, track)
    ...
    def get_pnt_ahead(self, track, pos, dist_forward, segidx, laneidx):
        """
        Gets point ahead.
        :param track: track-info
        ...
        if segidx == -1 or laneidx == -1:
            seg_, lane_ = get_index_seglane(pos, track.pnts_poly_track)
            segidx, laneidx = seg_[0], lane_[0]

        pnts_c_tmp = track.pnts_m_track[segidx][laneidx]  # [0, :] start --> [end, :] end
        pnts_c_tmp = interpolate_data(pnts_c_tmp, data_type=0, alpha=10)

        if segidx < (track.num_seg - 1):
            child_tmp = track.idx_child[segidx][laneidx]
            for nidx_tmp in range(0, len(child_tmp)):
                pnts_c_next_tmp = track.pnts_m_track[segidx + 1][track.idx_child[segidx][laneidx][nidx_tmp]]
                # [0, :] start --> [end, :] end
    ...
    # NAIVE CONTROL ---------------------------------------------------------------------------------------------------#
    def find_control_naive(self, track, u_set, horizon, data_ov, t, pose_goal, return_seglane=False):
        """
        Finds (naive) control.
        :param track: track-info
        :param u_set: set of controls (ndarray, dim = N x 2)
        ...
        for nidx_u in range(0, len_u_set):
            ...
            for nidx_h in range(1, h_horizon_c):
                ...
                seg_tmp_, lane_tmp_ = get_index_seglane(x_tmp[0:2], track.pnts_poly_track)
    ...
    def get_info_t_reward(self, track, t, pose_t, v_t, data_ov, use_intp=0):
        """
        ...
        seg_ev_t_, lane_ev_t_ = get_index_seglane(pose_t[0:2], track.pnts_poly_track)
        ...
        # Get feature
        f_t, id_near_t, _, _, _, dist_cf = get_feature(track, data_ev_t, data_ov_t, use_intp=use_intp)
    ...
    def compute_reward_component(self, track, t, data_ev_next, lv, w, data_ov_near, lane_dev_rad, lane_dev_dist_scaled, dist_cf):
        """
        ...
        dist2goal_, reach_goal_ = get_dist2goal(data_ev_next[1:3], data_ev_next[7], data_ev_next[8],
                                                track.indexes_goal, track.pnts_goal)
